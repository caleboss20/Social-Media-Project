...
function ConnectUsers() {
  ...
  return (
    <div className="flex mb-100 h-100 w-full bg-400 overflow-x-auto space-x-4 pl-4 pt-4 bg-[#fefefe] mt-0 rounded-xl">
        {users.map((item,i)=>
        <div className="relative border border-gray-200 flex-col flex-shrink-0 w-41 h-68 rounded-lg flex items-center justify-center text-[#212121]">
        <XMarkIcon onClick ={()=>handleDelete(i)} className="absolute top-2 right-2 w-6 h-6" />
        <img src={item.userimage} className="mt-3 w-20 h-20 rounded-full bg-blue-400" alt="" />
        <div className="flex flex-col justify-center items-center mt-4">
            <h2 className="text-black text-md font-bold">{item.username}</h2>
             <p className="mt-3 text-gray-500">suggested for you</p>
        </div>
        
        <button className="outline-none bg-blue-500 rounded-sm mt-7 font-bold px-8 py-3 text-md text-white"onClick={()=>toggleFollow(item)}>
